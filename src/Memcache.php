...
class Memcache implements DriverInterface
{
    ...
    public function __construct(array $config = [])
    {
        if (empty($config['type']) || !$this->check()) {
            throw new MemcacheException('Memcache(d) not installed or not select type');
        }
        ...
        switch (strtolower($config['type'])) {
            ...
            default:
                throw new MemcacheException('Selected type not valid in the driver');
        }
        ...
        if (!empty($config['servers'])) {
            ...
            throw new MemcacheException('Server(s) not configured');
        }
    }
